...
const tblPatientsBody = document.getElementById('tblCustomersBody'); 
const formEditPatient = document.getElementById('frmEditCustomers');  
document.addEventListener('DOMContentLoaded', () => {
  const form = document.getElementById('frmCustomers'); 
  const msg  = document.getElementById('msg');

  if (!form) {
    console.error('No existe #frmCustomers en el DOM');
    return;
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!payload.identification_number || !payload.name || !payload.email || !payload.phone || !payload.address) {
...
window.addEventListener('DOMContentLoaded', () => {
  const frm = document.getElementById('frmEditCustomer'); 
  if (!frm) return;
  ...
  frm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const identification_number = document.getElementById('customerIdentificationNumber').value.trim();
      const name  = document.getElementById('customerName').value.trim();
      const email = document.getElementById('customerEmail').value.trim();
      const phone = document.getElementById('customerPhone').value.trim();
      const city  = document.getElementById('customerCity').value.trim();

      await updateCustomer(id, { identification_number, name, email, phone, city });
...
async function showCustomer(id) {
  try {
    ...
    const modalElement = document.getElementById('exampleModalCustomer');
